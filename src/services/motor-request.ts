...
export async function trackMotorRequest(trackingNumber: string) {
  ...
  if (import.meta.env.DEV) {
    ...
    try {
      const engineeringTracking = await getJson<MotorRequestTracking>(
    ...
    try {
      const healthTracking = await getJson<MotorRequestTracking>(
        `/api/v1/public/health-requests/track/${encodedTrackingNumber}`,
      );
    ...
    try {
      const generalAccidentTracking = await getJson<MotorRequestTracking>(
        `/api/v1/public/general-accident-requests/track/${encodedTrackingNumber}`,
      );
    ...
    try {
      const transportTracking = await getJson<MotorRequestTracking>(
        `/api/v1/public/transport-requests/track/${encodedTrackingNumber}`,
      );

      return normalizeTrackingResponse(transportTracking, "transport");
    } catch (error) {
      if (!(error instanceof ApiError) || error.status !== 404) {
        throw error;
      }
    ...
    const transportTracking = await getJson<MotorRequestTracking>(
      `/api/v1/public/transport-requests/${encodedTrackingNumber}`,
    );
